...
public class TradeResponseCommand extends AMyCommand<Plugin> {
        ...
        if (args.length == 1) {
            ...
            if (target != null) {
                ...
                try {
                    ...
                    plugin.getLogger().warning("Error al obtener trades pendientes para autocompletado: " + e.getMessage());
                    return List.of();
                }
            }
        }
    ...
    private CompletableFuture<Boolean> validateTradeRequest(Player responder, Player requester, long tradeId) {
        if (requester == null || !requester.isOnline()) {
            responder.sendMessage(PLAYER_OFFLINE);
            return CompletableFuture.completedFuture(false);
        }
        ...
        return plugin.getTradeManager().isTradeValid(tradeId)
            .thenCompose(isValid -> {
                if (!isValid) {
                    responder.sendMessage(EXPIRED);
                    requester.sendMessage(EXPIRED_REQUESTER);
